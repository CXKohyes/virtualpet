...
| species | VARCHAR(16) | NOT NULL | CAT/DOG/DRAGON/RABBIT |
...
| 物种 | 倾向 | 战斗里的表现 |
| --- | --- | --- |
| 猫 | 速度 +6 / 攻击 +1 / 防御 +1 | 出手极快，靠速度差换追加攻击 |
| 狗 | 生命 +4 / 防御 +2 / 每回合回血 4% | 拖持久战 |
| 龙 | 生命 +6 / 攻击 +5 | 单次伤害最高，但没有恢复手段 |
| 兔子 | 速度 +7 / 防御 +1 | 全物种最快，但**攻击零加成** —— 靠先手而不是靠伤害 |

> 表里的猫和狗各有两个小编号加成，是原先就有的、上表漏记的（`Species.java` 为准）。

物种倾向的具体数值**随等级缩放**（5 级为基准）。固定加成在低等级占比过大、
高等级占比过小，实测会让同一个倾向在两端给出相反结论。

**平衡是靠实测调出来的，不是靠看公式。** 中期（5 级）所有物种两两都在 40–60%，
这条钉在 `BattleSimulatorTest.midGameIsBalanced` 里，且**遍历 `Species.values()`
而不是写字面量** —— 加物种时配对会从 3 对自动变成 6 对。

兔子的数值就是这么调出来的：第一版给的是「猫的加强版」（生命 +2、速度 +7），
实测把猫压到 26% 胜率。**速度在战斗里的权重远高于生命和攻击**，
所以兔子的速度要全物种最高，代价就必须从攻击上扣 —— 现在攻击零加成。

> **1 级和 10 级两端仍不平衡**，这是既有问题、不是本次引入的：实测 10 级
> 狗打龙 79%、猫打龙 68%，1 级也有几对落在 60% 上下。测试只钉 5 级
> （中期是玩家停留最久的一段）。兔子在这两端反而比龙更稳（10 级 40–71%）。
...
- 输出「物种数 × 3」张 64×64 RGBA PNG（当前四个物种，12 张）。
- 使用 16–24 色统一调色板。**这是加物种的硬上限**：每加一个物种要多占 3 个色阶，
  当前 23 色。要再加第五个物种，得先把「每个物种一套色」改成多物种共用调色板，
  否则脚本里的 `assert 16 <= len(PALETTE) <= 24` 会直接失败。
- 使用最近邻缩放，禁止抗锯齿。
- 生成 `contact-sheet.png` 用于一致性检查。
- 固定随机种子，脚本重复执行结果一致。
- 生成文件直接写入 `pet-web/src/assets/pets/`。

**新物种只能追加到 `SPECIES` 元组末尾，不能插在中间。** 整个脚本共用一个
`random.Random(SEED)`，按元组顺序依次喂给每个 `render()`，阶段 2 的星点会消耗它。
插在中间会让它后面所有物种的星点落点全变 —— 也就是「加了个物种」顺手改掉了
已入库的旧精灵图。追加在末尾则旧图逐字节不变，这条可以直接用
`git status pet-web/src/assets/pets/` 验。
...
- 狗：垂耳、圆口鼻、短尾。
- 龙：角、翅膀、尖尾。
- 兔子：长耳（4 像素宽、9 像素高，比猫的尖耳更高更窄）、小口鼻、圆球尾。
  骨架和猫一样，所以剪影之外还必须给脸部一个差异点，否则读起来就是「猫换个颜色」。
...
- `OnboardingView`：候选宠物展示、名字校验、创建成功。
