...
**养成主链路可以完整游玩**：匿名会话 → 领养命名 → 四项照护 → 离线衰减 →
生病与进化 → 照护日志。

**异步对战已打通**（P1）：好友码 → 发起挑战 → 服务端确定性自动战斗 → 战报 →
实时通知。被挑战方不需要在线，打的是开战那一刻的快照。

**四个物种已完成**（P2，2026-09）：在猫、狗、像素龙之外新增了兔子，领养页四选一。

**多宠物槽已完成**（P2 第一项，2026-09）：最多养 3 只，同一时间只有一只「当前宠物」，
主界面顶部的名册可以一眼看出谁快不行了，点一下切换。非活跃宠物**照常衰减**，
没有暂停机制 —— 三只都得上心。槽位 3 是在只有三个物种时定的，
物种加到四个之后一次养不下全部，玩家必须挑 —— 这是有意保留的取舍。

**公网部署已完成**：单机 systemd + nginx，见 `docs/deployment.md`。
接口限流和 WebSocket 来源白名单都配好了。

还没做的（按 `PRD.md` 的优先级）：

- **P0 欠账**：本机一键启动脚本（`scripts/dev.ps1`）—— 从批次 0 就写在设计文档里，
  至今没做。
- **P1 欠账**：动作帧（现在只有单帧图 + CSS 位移抖动）、每日照护摘要、回访报告补对战。
- **P2 剩余**：账号体系（已评估后搁置，理由见下）、道具与收集图鉴、
  更多物种与分支进化与季节活动、实时对战。
- **运维**：HTTPS/WSS（卡在 ICP 备案）、监控告警、数据库备份。

> **账号体系为什么搁置**：`PRD.md` 6.7 规定不收集邮箱和手机号，这同时堵死了
> 密码找回。没有找回通道的密码登录会变成半成品 —— 忘记密码就等于账号在那台设备上
> 永久锁死，比没有账号更难解释。真要做，得先决定是否放宽隐私范围。

接口契约见 `docs/api.md`，部署见 `docs/deployment.md`。
...
├── deploy\              部署产物：nginx 配置、systemd 单元、服务器初始化脚本
├── scripts\
│   ├── generate_sprites.py   程序化生成精灵图（不要手改产物）
...
精灵图是脚本画出来的（物种数 × 3 个形态），**不要手改产物**，要调就改脚本再跑一次：
