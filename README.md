...
已完成 **批次 0–4**：工程骨架、领域规则、数据与接口、前端主流程、内容与打磨。
MVP 的养成主链路已经可以完整游玩：匿名会话 → 领养命名 → 四项照护 → 离线衰减 →
生病与进化 → 照护日志。**尚未实现异步对战（批次 6，P1）。**
...
Vite 已配置开发代理：前端请求 `/api` 会被转发到 `http://localhost:8080`，因此开发期不需要额外处理跨域。
